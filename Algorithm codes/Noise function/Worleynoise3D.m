...
feature_points_map = cell(f, f, f);
for ux=0:f-1
    for uy=0:f-1
        for uz=0:f-1
            randobject= RandStream('mt19937ar', 'Seed', seed(uy+1, ux+1, uz+1));
            feature_points_map{uy+1, ux+1, uz+1}= rand(randobject, n, 3) + [ux, uy, uz];
        end
    end
end
...
for x=f/m:f/m:f
    ...
    for y=f/m:f/m:f
        ...
        for z=f/m:f/m:f
            ...
            feature_points= feature_points_map{uy+1, ux+1, uz+1};
            for di= -1:1
                for dj= -1:1
                    for dk= -1:1
                        nx= ux + di;
                        ...
                        if di ~= 0 || dj ~= 0 || dk ~= 0
                            feature_points= [feature_points; ...
                                              feature_points_map{ny+1, nx+1, nz+1}];
                        end
                    end
                end
            end
